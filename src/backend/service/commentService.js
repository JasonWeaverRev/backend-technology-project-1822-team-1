...
async function updateComment(
  ...
  console.log(
    `Updating comment with ID: ${comment_id}, creation_time: ${comment_creation_time}, by user: ${username}`
  );

  // Fetch the comment to ensure it exists
  const comment = await commentDao.getCommentById(
    comment_id,
    comment_creation_time
  );
  ...
  const result = await commentDao.updateCommentByUser(
    comment_id,
    comment_creation_time,
    body
  );
...
const deleteComment = async (post_id, creation_time, username) => {
  console.log(
    `Deleting comment with ID: ${post_id}, creation_time: ${creation_time}, username: ${username}`
  );
  ...
  // Proceed to delete
  const deleteResult = await commentDao.deleteCommentByUser(
    post_id,
    creation_time
  );
  return deleteResult;
